/**
 * NIC — Set de íconos SVG outlined (line-icons, estilo Lucide/Feather).
 * Todos usan stroke="currentColor" y fill="none" => heredan el color del texto
 * contenedor (botón, chip, título…). Sin emojis en ninguna parte de la UI.
 *
 * Uso:  icon('home')  ó  icon('droplet', { size: 18 })
 */
...
const PATHS = {
  ...
  // Ambiente / energía / planta / IA
  droplets: '<path d="M7 16.3c2.2 0 4-1.83 4-4.05 0-1.16-.57-2.26-1.71-3.19S7.29 4.7 7 2.75c-.29 1.95-1.14 3.38-2.29 4.31S3 11.1 3 12.25c0 2.22 1.8 4.05 4 4.05z"/><path d="M12.56 6.6A10.97 10.97 0 0 0 14 3.02c.5 2.5 2 4.9 4 6.5s3 3.5 3 5.5a6.98 6.98 0 0 1-11.91 4.97"/>',
...
/**
 * Devuelve el markup SVG de un ícono outlined.
 * @param {string} name  clave del ícono
 * @param {{ size?: number, sw?: number, cls?: string }} [opts]
 */
export function icon(name, opts = {}) {
  const body = PATHS[name];
  if (!body) return '';
  const { size = 22, sw = 2, cls = '' } = opts;
